...
# --- LOAD DATA TỪ GOOGLE SHEETS & TỰ ĐỘNG CHẤM ĐIỂM BẤT KỂ LÚC NÀO ---
def fetch_data_from_sheet(raw_url):
    ...
    try:
        ...
        # TỰ ĐỘNG CHẤM ĐIỂM TẤT CẢ CÁC DÒNG NGAY KHI TẢI DỮ LIỆU SANG
        df['Điểm'] = 0
        for idx, row in df.iterrows():
            ...
            if col_name and pd.notna(row[col_name]):
                df.at[idx, 'Điểm'] = score_lead(str(row[col_name]))
...
with st.sidebar:
    ...
    st.info("🍊 Tone màu Cam Premium UI & Tự động chấm điểm 100%.")
    st.divider()
...
if reload_btn or 'df' not in st.session_state or st.session_state.get('loaded_url') != sheet_url:
    ...
    if err:
        ...
        st.sidebar.success("✅ Đã kết nối & Tự động chấm điểm xong!")
...
with col2:
    if st.button("🔄 Tải & Chấm lại điểm", use_container_width=True, type="primary"):
        with st.spinner("Đang quét và chấm lại điểm..."):
            for idx, row in df.iterrows():
                col_name = 'Mô tả nhu cầu' if 'Mô tả nhu cầu' in row else ('nhu_cau_mo_ta' if 'nhu_cau_mo_ta' in row else None)
                if col_name and pd.notna(row[col_name]):
